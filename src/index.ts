...
patches.push(before("dispatch", FluxDispatcher, ([event]) => {
  if (event.type === "MESSAGE_DELETE") {
    ...
    // Sprawdzenie czy wiadomość jest od zalogowanego użytkownika
    const currentUser = UserStore.getCurrentUser();
    if (storage.ignoreOwnDeleted && message.author?.id === currentUser?.id) {
      return event; // Pozwól na normalne usunięcie własnych wiadomości
    }
